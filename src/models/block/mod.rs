...
pub struct Block {
    ...
    /// Merkle update for the shard state.
    pub state_data_updates: Dict<u32, LazyExotic<MerkleUpdate>>,
...
impl Block {
    ...
    pub fn iter_state_data_updates(&'_ self) -> StateDataUpdateIter<'_> {
        StateDataUpdateIter {
            inner: self.state_data_updates.raw_values(),
        }
    }
...
impl Iterator for StateDataUpdateIter<'_> {
    type Item = Result<Lazy<MerkleUpdate>, Error>;

    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next()? {
            Ok(mut value) => {
                let e = match value.load_reference_as_slice() {
                    Ok(mut value) => match Lazy::load_from(&mut value) {
                        Ok(merkle_update) => return Some(Ok(merkle_update)),
                        Err(e) => e,
                    },
                    Err(e) => e,
                };
