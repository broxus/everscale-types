...
mod tests {
    ...
    fn boc_with_crc() {
        ...
        let cell = Boc::<RcCellFamily>::decode(&boc_without_crc).unwrap();

        let mut boc_with_crc = Vec::new();
        ser::BocHeader::<RcCellFamily, ahash::RandomState>::new(cell.as_ref())
            .with_crc(true)
            .encode(&mut boc_with_crc);
        assert_eq!(boc_without_crc.len() + 4, boc_with_crc.len());

        let decoded = Boc::<RcCellFamily>::decode(&boc_with_crc).unwrap();
        ...
        *last_byte = !*last_byte;
        ...
        assert!(matches!(
            Boc::<RcCellFamily>::decode(&boc_with_crc),
